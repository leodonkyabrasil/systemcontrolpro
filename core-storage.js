...
// Sobrescreve empresas/usuarios do localStorage a partir de arquivos JSON (sem cache)
async function forceReloadFromFiles(){
  try{
    const ej = await fetch('./empresas.json', { cache: 'no-store' });
    const ejj = await ej.json();
    const uj = await fetch('./usuarios.json', { cache: 'no-store' });
    const ujj = await uj.json();

    const empresas = Array.isArray(ejj?.empresas) ? ejj.empresas : [];
    const usuariosFlat = Array.isArray(ujj?.usuarios) ? ujj.usuarios
                        : (Array.isArray(ujj) ? ujj : []);

    setData('empresas', empresas);
    setData('usuarios', usuariosFlat);
    console.log('[SCP] forceReloadFromFiles: empresas/usuarios recarregados dos arquivos.');
  }catch(e){
    console.warn('[SCP] forceReloadFromFiles falhou:', e);
  }
}
